import { Injectable, NotFoundException, Inject, Logger, InternalServerErrorException } from "@nestjs/common";
...
export class UsersRepositoryImpl implements UsersRepository {
  ...
  async create(user: IUser): Promise<IAuth> {
    try {
      ...
      throw new InternalServerErrorException('Error al intentar crear el usuario');
    }
  }
  ...
  async findAll(limit?: number, offset?: number): Promise<{ users: IUser[], meta: IMeta }> {
    try {
      ...
      throw new InternalServerErrorException('Error al intentar obtener usuarios');
    }
  }
  ...
  async update(id: string, userUpdate: IUser): Promise<IUser> {
    ...
    try {
      ...
      throw new InternalServerErrorException('Error al intentar editar el usuario');
    }
  }
  ...
  async verifyPhone(phone: string): Promise<void> {
    try {
      ...
      throw new InternalServerErrorException('Error al intentar verificar el telefono');
    }
  }
