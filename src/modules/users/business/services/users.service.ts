...
export class UsersService {
  ...
  async createFirstAdmin(createUserDto: CreateUserDto): Promise<IAuth> {
    ...
    const { password, ...user } = createUserDto;
    return this.userRepo.createFirstAdmin({
      ...user,
      password: bcrypt.hashSync(password, 10),
    });
  }
  ...
  async update(
    ...
    if (user.email)
      await this.userValidator.validateEmailUniqueness(user.email, userId);
    if (user.phone)
      await this.userValidator.validatePhoneUniqueness(user.phone, userId);
